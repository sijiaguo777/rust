use micromath::F32Ext as _;
use core::ops::Index;
use core::ops::{Mul, Div};
use core::ops::IndexMut;
use core::mem::transmute;

pub mod image{
    #[derive(Debug, Copy, Clone, Default)]
    #[repr(C)]
    pub struct Color {
        pub r: u8,
        pub g: u8,
        pub b: u8,
    }
    #[repr(transparent)]
    pub struct Image(pub [Color; 64]); // An image is a collection of 64 Color pixels
}


impl image::Image {
    pub fn new_solid(color: image::Color) -> Self {
        Self([color; 64])
    }
    fn default() -> Self {
        Self([image::Color::default(); 64])
    }
    pub fn row(&self, row: usize) -> &[image::Color] {
        &self.0[row * 8..(row + 1) * 8]
    }

    pub fn gradient(color: image::Color) -> Self
    {
        let mut image = Self::default();
        for row in 0..8 {
            for col in 0..8 {
                image[(row, col)] = color / (1 + row * row + col) as f32;
            }
        }
        image
    }
}


pub mod gamma{
    pub const GAMMA_TAB: [u8; 256] = [
    ,];

    pub fn gamma_correct(x: u8) -> u8 {
        GAMMA_TAB[x as usize]
    }
}


impl image::Color {
    pub const RED: Self = Self{r:255, g:0, b:0};
    pub const GREEN: Self = Self{r:0, g:255, b:0};
    pub const BLUE: Self = Self{r:0, g:0, b:255};

    pub fn gamma_correct(&self) -> Self {
        Self {
            r: gamma::gamma_correct(self.r),
            g: gamma::gamma_correct(self.g),
            b: gamma::gamma_correct(self.b),
        }
        
    }
}


impl Mul<f32> for image::Color {
    type Output = Self;
    fn mul(self, scalar: f32) -> Self {
        Self {
            r: (self.r as f32 * scalar).round().clamp(0.0, 255.0) as u8,
            g: (self.g as f32 * scalar).round().clamp(0.0, 255.0) as u8,
            b: (self.b as f32 * scalar).round().clamp(0.0, 255.0) as u8,
        }
    }
}


impl Div<f32> for image::Color {
    type Output = Self;
    fn div(self, scalar: f32) -> Self {
        self * (1.0 / scalar)
    }
}


const WIDTH: usize = 8;
impl Index<(usize, usize)> for image::Image {
    type Output = image::Color;
    fn index(&self, (row, col): (usize,usize)) -> &Self::Output {
        let idx = row * WIDTH + col;
        &self.0[idx] // self.0 is the array of colors
    }
}


impl IndexMut<(usize, usize)> for image::Image {
    fn index_mut(&mut self, (row, col): (usize,usize)) -> &mut Self::Output {
        let idx = row * WIDTH + col;
        &mut self.0[idx]
    }
}

impl AsRef<[u8; 192]> for image::Image {
    fn as_ref(&self) -> &[u8; 192] {
        unsafe {transmute(&self.0)}
    }
}


impl AsMut<[u8; 192]> for image::Image {
    fn as_mut(&mut self) -> &mut [u8; 192] {
        unsafe {transmute(&mut self.0)}
    }
}


fn main() {
    let color = image::Color {r: 200, g: 2, b: 1};
    let divided_color = color / 2.0;
    println!("{:?}", divided_color); 
}



